...
<style type = "text/css">
	...
	.step-panels{
		...
		#qnaTable tr.viewImg:LAST-CHILD{
			border-bottom: none;
		}
		#qnaTable tr > *{
			text-align : center;
	...
	<div class= "qnaList">
		<table id = "qnaTable">
			...
			<c:if test="${not empty qnaList }">
				<c:forEach items="${qnaList }" varStatus="status">
					<tr class = "hahaha">
						<td class=  "viewImg">
							<a href = "/detail?no=${qnaList[status.index].p_no }"><img src = "/productUpload/${qnaList[status.index].pi_file }"></a>
						</td>
						<td>${qnaList[status.index].q_title }</td>
						<td>${qnaList[status.index].q_date }</td>
					</tr>
					<tr class = "clickEvent">
						<td colspan="3">${qnaList[status.index].q_content }</td>
					</tr>
				</c:forEach>
				<tr class= "pagingNumbers">
							<td colspan="3" id = "pageLow">
								...
								<c:if test = "${memberVO.pageTotal[0] eq 1 and pNum ne 1}" >
									<a href = "/member/mypage/qna?pageNum=1" data-num = "1" class = "icon-angle-double-left"></a>
								</c:if>
								...
								<c:if test = "${memberVO.pageTotal[0] ne 1}" >
									<a href = "/member/mypage/qna?pageNum=${memberVO.pageTotal[0] - fn:length(memberVO.pageTotal) }" data-num = "${memberVO.pageTotal[0] - fn:length(memberVO.pageTotal) }" class = "icon-angle-left"></a>
								</c:if>
								...
								<c:if test = "${memberVO.pageTotal[fn:length(memberVO.pageTotal) - 1] ne memberVO.totalPage}" >
									<a href = "/member/mypage/qna?pageNum=${memberVO.pageTotal[0] + fn:length(memberVO.pageTotal) }" data-num = "${memberVO.pageTotal[0] + fn:length(memberVO.pageTotal) }" class = "icon-angle-right"></a>
								</c:if>
								...
								<c:if test = "${memberVO.pageTotal[fn:length(memberVO.pageTotal) - 1] eq memberVO.totalPage and memberVO.totalPage ne pNum}" >
									<a href = "/member/mypage/qna?pageNum=${memberVO.totalPage }" data-num = "${memberVO.totalPage }" class = "icon-angle-double-right"></a>
								</c:if>
								<c:if test = "${memberVO.pageTotal[fn:length(memberVO.pageTotal) - 1] ne memberVO.totalPage}" >
									<a href = "/member/mypage/qna?pageNum=${memberVO.totalPage }" data-num = "${memberVO.totalPage }" class = "icon-angle-double-right"></a>
								</c:if>
